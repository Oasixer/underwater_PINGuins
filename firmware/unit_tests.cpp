...
bool test_ring_buffer_read_and_replace(){
    ...
    if (is_initialization_good){
        Serial.println("Ring buffer Initialization Test Passed");
    } else {
        Serial.println("Ring buffer Initialization Test Failed");
    }
    ...
    for (uint16_t i = 0; i < SZ_WINDOW; ++i){
        ...
        is_read_and_replace_good &= ring_buffer.i_curr == (i+1) % SZ_WINDOW;
    }
    for (uint16_t i = 0; i < SZ_WINDOW; ++i){
    ...
    if (is_read_and_replace_good){
        Serial.println("Ring buffer Read and Replace Test Passed");
    } else {
        Serial.println("Ring buffer Read and Replace Test Failed");
    }

    bool all_tests_pass = is_read_and_replace_good && is_initialization_good;
    if (all_tests_pass){
        Serial.println("Ring buffer all tests Passed\n");
    } else {
        Serial.println("Ring buffer tests Failed\n");
    }
...
bool test_complex_type(){
    ...
    if (is_magnitude_working){
        Serial.println("Complex Magnitude Test Passed");
    } else {
        Serial.println("Complex Magnitude Test Failed");
    }

    complex_t sum = complex_add_real(c1, 5);
    bool is_addition_working = sum.real == 8 && sum.imaginary == 4;
    if (is_addition_working){
        Serial.println("Complex Addition Test Passed");
    } else {
        Serial.println("Complex Addition Test Failed");
    }
    ...
    if (is_multiplication_woring){
        Serial.println("Complex Multiplication Test Passed");
    } else {
        Serial.println("Complex Multiplication Test Failed");
    }

    bool all_tests_pass = is_magnitude_working && is_addition_working && is_multiplication_woring;
    if (all_tests_pass){
        Serial.println("Complex all tests Passed\n");
    } else {
        Serial.println("Complex tests Failed\n");
    }
...
bool test_fourier(){
    ...
    for (uint8_t i = 0; i < N_FREQUENCIES; ++i){
        is_no_signal_working &= amplitudes[i] < MAX_FOURIER_ROUNDING_ERROR; // should be low
    }
    if (is_no_signal_working){
        Serial.println("Fourier No Signal Test Passed");
    } else {
        Serial.println("Fourier No Signal Test Failed");
    }
    ...
    for (uint8_t i_freq = 0; i_freq < N_FREQUENCIES; ++ i_freq){
        ...
        // fourier_print_ring_buffer();
        for (int j = 0; j < 6; ++j){
            if (i_freq != j){  // should be low
                is_individual_working &= amplitudes[j] < MAX_FOURIER_ROUNDING_ERROR;
            } else {  // should be high
                is_individual_working &= fabsf(amplitudes[j] - 1280000) < MAX_FOURIER_ROUNDING_ERROR;
            }
        }
    }
    if (is_individual_working){
        Serial.println("Fourier Individual Test Passed");
    } else {
        Serial.println("Fourier Individual Test Failed");
    }
    ...
    for (uint8_t i = 0; i < N_FREQUENCIES; ++i){
        is_multiple_frequencies_high_working &= fabs(amplitudes[i] - 213333.333333) < MAX_FOURIER_ROUNDING_ERROR; // should be high
    }
    if (is_multiple_frequencies_high_working){
        Serial.println("Fourier Multiple High Test Passed");
    } else {
        Serial.println("Fourier Multiple High Test Failed");
    }

    bool all_tests_pass = is_no_signal_working && is_individual_working && is_multiple_frequencies_high_working;
    if (all_tests_pass){
        Serial.println("Fourier all tests Passed\n");
    } else {
        Serial.println("Fourier tests Failed\n");
    }
...
//                    Serial.println("coords_3d: (%.2f, %.2f, %.2f), (%.2f, %.2f, %.2f), (%.2f, %.2f, %.2f), (%.2f, %.2f, %.2f)",
...
//                    Serial.println("dists3d: %.2f, %.2f, %.2f, estimate: (%.2f, %.2f, %.2f), actual: (%.2f, %.2f, %.2f)\n",
...
//                           rov_coord.x, rov_coord.y, rov_coord.z);
    ...
    if (quadrant_tests_pass){
        Serial.println("Multilateration quadrant tests Passed");
    } else {
        Serial.println("Multilateration quadrant tests Failed");
    }
    ...
    if (stationary_collinear_test_passed){
        Serial.println("Multilateration stationary collinear tests Passed");
    } else {
        Serial.println("Multilateration stationary collinear tests Failed");
    }
    ...
    if (rov_collinear_test_passed){
        Serial.println("Multilateration ROV collinear tests Passed");
    } else {
        Serial.println("Multilateration ROV collinear tests Failed");
    }
    ...
    if (superposition_test_passed){
        Serial.println("Multilateration superposition tests Passed");
    } else {
        Serial.println("Multilateration superposition tests Failed");
    }
    ...
    if (all_tests_pass){
        Serial.println("Multilateration all tests Passed\n");
    } else {
        Serial.println("Multilateration tests Failed\n");
    }
